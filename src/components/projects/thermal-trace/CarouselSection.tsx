  ...
  return <>
      ...
      <div className="w-full mb-40">
        ...
        <div className="flex justify-center space-x-2 mt-6">
          {images.map((_, index) => <div key={index} className={`w-6 h-0.5 cursor-pointer transition-all duration-300 ${current === index ? 'bg-white' : 'bg-white/40 hover:bg-white/70'}`} onClick={() => api?.scrollTo(index)} />)}
        </div>
      </div>
    </>;
};
