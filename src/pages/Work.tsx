          ...
          <section className="relative h-[calc(100vh-200px)] overflow-hidden">
            <div className="relative w-full h-full">
              {projects.map((project, index) => (
                <div key={project.id} className={`absolute inset-0 transition-all duration-[2000ms] ease-[cubic-bezier(0.25,0.46,0.45,0.94)] ${index === currentSlide ? 'opacity-100 z-10' : 'opacity-0 z-0'}`}>
          ...
          <section className="py-8 px-4 md:px-8">
            <div className="max-w-7xl mx-auto">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {projects.map((project, index) => (
                  <div key={project.id} className="opacity-100 translate-y-0" style={{
                    ...
                    <Link to={`/project/${project.slug}`} className="group relative overflow-hidden bg-gray-900 rounded-lg transition-all duration-[1000ms] ease-[cubic-bezier(0.25,0.46,0.45,0.94)] hover:transform hover:scale-105 block">
                      ...
                      {/* Main Tags */}
                      <div className="absolute top-4 left-4 right-16 z-20 flex flex-wrap gap-2">
                      ...
                      {/* Project Number */}
                      <div className="absolute top-4 right-4 w-8 h-8 bg-white/10 backdrop-blur-sm rounded-full flex items-center justify-center z-30">
